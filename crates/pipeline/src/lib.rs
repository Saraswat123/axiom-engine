...
impl OptVerify {
    ...
    pub async fn run(&self, expr: &str, domain: Option<(i64, i64)>) -> Result<OptVerifyResult> {
        ...
        if let Some(cached) = self.store.get(&cache_key) {
            ...
            return Ok(OptVerifyResult { cache_hit: true, ..result });
        }

        // Step 1: egg — equality saturation
        let egg_input = json!({ "expression": expr });
        let egg_out = self.trace.record("egg_optimize", egg_input.clone(), |inp| {
            let egg = EggTool::new();
            async move { egg.optimize(inp).await }
        }).await?;

        let optimized = egg_out["optimized"]
            .as_str()
            .unwrap_or(expr)
            .to_string();
        ...
        let z3_out = self.trace.record("z3_prove", z3_input.clone(), |inp| {
            let z3 = Z3Tool::new();
        ...
        self.store.insert(cache_key, axiom_store::Artifact {
